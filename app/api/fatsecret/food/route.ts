...
export async function GET(req: NextRequest) {
  ...
  if (!fatSecretEnabled) {
    return NextResponse.json(
      {
        error: 'fatsecret_disabled',
        message: 'FatSecret integration is currently disabled.',
        enabled: false,
      },
      {
        status: 503,
  ...
  try {
    ...
    const servingList = Array.isArray(
      rawServings,
    )
      ? rawServings
      : rawServings
    ...
    const servings = servingList
      .map((rawServing, index) => {
        const serving =
          rawServing as Record<
            string,
            unknown
          >;

        const kcal = numberValue(
          serving.calories,
        );
