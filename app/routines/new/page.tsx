...
const DEFAULT_ITEM = (order: number): DraftItem => ({
  order,
  exercise_name: '',
  target_sets: 3,
  target_reps: 10,
...
function repsLabel(recordType: RecordType) {
  return recordType === 'time' ? '목표(초)' : '목표 횟수';
}
...
export default function NewRoutinePage() {
  ...
  // Consume pending exercise selection from library picker
  useEffect(() => {
  ...
  const addItem = () => {
    setItems(prev => [...prev, DEFAULT_ITEM(prev.length)]);
  };
      ...
      <form onSubmit={handleSubmit} className="px-4 pt-4 space-y-6">
        ...
        <div>
          ...
          <input type="text" value={name} onChange={e => setName(e.target.value)}
            ...
            placeholder="예: 가슴·삼두, 풀데이, PPL-Push" autoFocus />
        </div>
