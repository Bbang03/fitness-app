      ...
      <div className="flex-1">
        ...
        <div className="space-y-2 mb-8">
          {exercises.map((ex, i) => (
            <div
              key={i}
              className="bg-zinc-900 rounded-xl px-4 py-3 flex items-center gap-3"
            >
...
function RestScreen({
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 bg-zinc-950">
      <p className="text-zinc-400 text-sm mb-8 uppercase tracking-widest">
        휴식 중
      </p>
...
function ExerciseScreen({
  ...
  const prevDefaultWeight =
    lastCompleted
      ? String(
          lastCompleted.weight_kg,
        )
      : previousSet
        ? String(
  ...
  const [weight, setWeight] =
    useState(prevDefaultWeight);

  const [reps, setReps] =
    useState(String(targetReps));
          ...
          <div className="flex flex-wrap gap-2">
            {completedSets.map(
              (s) => (
                <div
                  key={s.id}
                  className="bg-emerald-900/30 border border-emerald-700/40 rounded-xl px-3 py-2 flex items-center gap-1.5"
                >
                  <Check
                    size={12}
  ...
  return (
    <div className="min-h-screen flex flex-col px-4 pt-12 pb-10">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          ...
          ({
            ...
            <div
              key={label}
              className="bg-zinc-900 rounded-2xl p-3 text-center"
            >
              ...
              <div className="flex flex-wrap gap-1.5">
                {sets.map(
                  (s) => (
                    <span
                      key={s.id}
                      className="text-xs bg-zinc-800 text-zinc-300 px-2.5 py-1 rounded-lg"
                    >
                      {setLabel(
                        s,
                        s.record_type ??
      ...
      <div className="mt-auto">
        {saveError && (
          <p className="text-red-400 text-sm bg-red-900/20 rounded-xl px-3 py-2 text-center mb-3">
            {saveError}
          </p>
        )}
        ...
        <button
          ...
          className={`w-full text-white font-bold py-4 rounded-2xl text-lg transition-colors ${
            isSaving
...
export default function WorkoutPage() {
  ...
  const handleResume =
    useCallback(() => {
      // activeWorkout이 이미 Zustand에 있으므로
      // 별도 작업 없이 현재 화면을 계속 사용
    }, []);
